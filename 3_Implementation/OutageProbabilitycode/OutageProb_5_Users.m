...
tau_4 = R_5./ (SNR_linear * (a5-a1 *R_4));
...
Outage_P5 = 1 - Omega_1 * exp( - ( (tau_4/Omega_1 ) + ( R_5./ (SNR_linear * Omega_2 ) ) ) ) ./ ...
                    (Omega_1 + SNR_linear .* varpi .* tau_4 * Omega_LI);
%Plotting 

figure;
semilogy(SNR_dB, Outage_P1, '-sr', SNR_dB, Outage_P2, '-og', SNR_dB, Outage_P3, '->k', SNR_dB, Outage_P4, '-pb', SNR_dB, Outage_P5, '-<r');
grid on;

xlabel('SNR (dB)');
ylabel('Outage Probability');
title('Outage Probability vs SNR Plot for five users');
legend({'User1 - NOMA','User2 -NOMA','User3 -NOMA','User4 -NOMA','User5 -NOMA'}, 'Location', 'Best', 'FontSize', 15);
